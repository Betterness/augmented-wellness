...
type BetterXPolicyConfig = {
  enabled?: boolean;
  appAliases?: string[];
  actionGatePaths?: string[];
};
...
function argvFromToolParams(params: Record<string, unknown>): string[] {
...
function verbFromArgv(argv: string[]): string {
  let i = 0;
  while (i < argv.length) {
    const part = argv[i] ?? "";
...
function commandMentionsXurl(text: string): boolean {
  return /(^|\s)(xurl|\/opt\/homebrew\/bin\/xurl|\/usr\/local\/bin\/xurl)(\s|$)/.test(text);
}

function commandUsesBetterXApp(text: string, appAliases: string[]): boolean {
...
function commandUsesActionGate(text: string, actionGatePaths: string[]): boolean {
...
export default definePluginEntry({
  ...
  register(api) {
    api.on(
      "before_tool_call",
      async (event) => {
        const cfg = (event.context?.pluginConfig ?? {}) as BetterXPolicyConfig;
